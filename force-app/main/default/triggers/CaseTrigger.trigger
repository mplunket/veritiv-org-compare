trigger CaseTrigger on Case (before insert,after insert, before update, after update) {
        ...
        if(trigger.isUpdate){
            ...
            if(CaseTriggerHandler.isFirstTimeCaseTriggerBUpdate){ 
               ...
               /* for(case cs: trigger.new){
                ...
                if(csList.size() > 0){
                    caseTriggerHandler.blockCases(csList);
                }*/
                for(case cs : trigger.new){ 
                ...
                //Logic to block cases
              // CaseTriggerHandler.blockCases(trigger.new);
                //End of logic to block cases
                ...
                //CaseServices.processOutOfOfficeCases( trigger.new );
                ...
                if(caseRecd.ContactId==null && caseRecd.SuppliedEmail!=null){
                    {
                        ...
                        {
                            /*
                            caseRecd.ContactId = mapContactbyEmail.get(caseRecd.SuppliedEmail).Id;
                            caseRecd.AccountId = mapContactbyEmail.get(caseRecd.SuppliedEmail).AccountId;
                            */
                        }
                    }
                }
    ...
    {    
        ...
        if ( !CaseNotification.caseTriggerAfterIsRunning ) {
            /*
                CaseNotification.caseTriggerAfterIsRunning = true;

                String USER_PREFIX = Schema.SObjectType.User.getKeyPrefix();

                List<Case> casesWithNewOwners = new List<Case>();

                for ( Case aCase : trigger.new )
                {
                    String ownerPrefix = String.valueOf(aCase.OwnerId).substring(0, 3);
                    //ensure that the owner has changed, it is a user, and the user isn't assigning it to themselves.
                    if ( ( aCase.OwnerId != trigger.oldMap.get( aCase.Id ).OwnerId )
                            && ownerPrefix == USER_PREFIX
                            && UserInfo.getUserId() != aCase.OwnerId )
                        casesWithNewOwners.add( aCase );
                }

                if ( casesWithNewOwners.size() > 0 )
                    CaseNotification.sendOwnerChangedEmail( casesWithNewOwners );
            */
     //   }
    //}
        }

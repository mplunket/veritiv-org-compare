trigger UserTrigger on User (before insert, after insert, after update, before update) {
    ...
    if(trigger.isUpdate && trigger.isAfter){
        ...
        //backlog 3030
        //check if user has been turned to inactive
        List<Id> inactiveUserIds = new List<Id>();
        for(User Usr: trigger.new){
            if(trigger.oldMap.get(Usr.Id).isactive != trigger.newMap.get(Usr.Id).isactive && trigger.newMap.get(Usr.Id).isactive == false){
                  inactiveUserIds.add(Usr.id);
                
            }
        }
        userservice.userChangedToInactive(inactiveUserIds);
    }
}